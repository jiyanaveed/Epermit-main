...
async function takeSubmitScreenshot(page, stepName) {
  try {
    ...
    console.log(`  [Submit] Screenshot captured: ${stepName} (${Math.round(base64.length / 1024)}KB)`);
    return {
      step_name: stepName,
...
async function permitWizardSubmit(sessionToken, filingData, supabase) {
  console.log("  [Submit] Starting PermitWizard submission finalization");
