...
- **Duplicate detection**: MD5 hash computed for every downloaded file. Session-level `_downloadedHashes` Map tracks content hashes to detect and log when identical files are downloaded under different names. `isDuplicate()` skips upload entirely for duplicate hashes and returns `skippedDuplicate: true`.
...
- **Browser crash resilience (March 2026)**: Long scrapes no longer crash due to accumulated RAM.
  - **Mini-reset**: Every 10 file downloads, the scraper closes the current page and opens a fresh one in the same browser context (preserving cookies), then re-navigates to the files tab and current folder. Clears page-level JS heap and DOM.
  ...
  - **Folder-level recovery**: Before clicking each folder, `isPageAlive()` checks if the page is responsive. If dead, recovery is attempted. If a folder crashes mid-scrape, recovery is attempted before the next folder; if unrecoverable, remaining folders are marked with `folderError: "browser_crashed"`.
