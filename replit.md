...
- **File storage**: Downloaded files are uploaded to Supabase Storage bucket `Project Drawings` at path `drawings/${projectId}/${fileName}`. Public URLs stored as `viewUrl` in `portal_data`. Local file deleted after successful upload; kept as backup if upload fails. Bucket auto-created on first use. `ensureStorageBucket()` auto-sets the bucket to public if not already. File paths are sanitized via `sanitizeStorageKey()` (spaces→underscores, special chars stripped).
